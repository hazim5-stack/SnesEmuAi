...
bool CXAudio2::InitVoices(void)
{
	...
	// XAudio2 2.8+ uses device ID string, not index
	const wchar_t *deviceId = nullptr;
	std::vector<std::wstring> devList = GetDeviceList();
	if (device_index > 0 && device_index < (int)devList.size())
		deviceId = devList[device_index].c_str();
...
std::vector<std::wstring> CXAudio2::GetDeviceList()
{
    ...
    // Use IMMDeviceEnumerator (XAudio2 2.8+ - Windows 8 SDK and later)
            ...
            for (UINT i = 0; i < count; i++)
            {
                ...
                if (SUCCEEDED(pColl->Item(i, &pDevice)))
                {
                    ...
                    if (SUCCEEDED(pDevice->OpenPropertyStore(STGM_READ, &pProps)))
                    {
                        ...
                        {
                            device_list.push_back(var.pwszVal);
