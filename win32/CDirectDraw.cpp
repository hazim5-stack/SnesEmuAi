...
bool CDirectDraw::Initialize (HWND hWnd)
{
    ...
    {
        MessageBox( GUI.hWnd, Languages[ GUI.Language].errModeDD, TEXT("SNES Emu Ai - DirectDraw(7)"), MB_OK | MB_ICONSTOP);
        return (false);
    }
...
bool CDirectDraw::SetFullscreen(bool fullscreen)
{
	...
	{
        MessageBox( GUI.hWnd, Languages[ GUI.Language].errModeDD, TEXT("SNES Emu Ai - DirectDraw(2)"), MB_OK | MB_ICONSTOP);
		return false;
	}
	return true;
}
