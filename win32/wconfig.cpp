...
static bool S9xSaveConfigFile(ConfigFile &conf){
	...
	configMutex = CreateMutex(NULL, FALSE, TEXT("SnesEmuAiConfigMutex"));
	std::string	fname;
...
#define	MAX_PARAMETERS 100
	...
	// ensure previous config file is not lost if we crashed while writing a new one
	{
		...
		if(tempfile)
		{
			...
			{
				fname=S9xGetDirectory(DEFAULT_DIR);
				if(i == 0)      fname+=SLASH_STR "SnesEmuAi.conf";
				else if(i == 1) fname+=SLASH_STR "SnesEmuAi.cfg";
...
#define CATEGORY "Config"
	...
	AddBoolC("Lock", readOnlyConfig, false, "if true, prevents SNES Emu Ai from editing this configuration file (or making it read-only while it is running)");
#undef CATEGORY
...
#define CATEGORY "Display\\Win"
	...
	AddBoolC("Fullscreen:EmulateFullscreen", GUI.EmulateFullscreen, true,"true makes SNES Emu Ai create a window that spans the entire screen when going fullscreen");
...
#define CATEGORY "Settings\\Win"
	...
	AddBoolC("PauseWhenInactive", GUI.InactivePause, TRUE, "true to pause SNES Emu Ai when it is not the active window");
...
#define	CATEGORY "Sound"
	...
	AddUIntC("InputRate", Settings.SoundInputRate, 32040, "for each 'Input rate' samples generated by the SNES, 'Playback rate' samples will produced. If you experience crackling you can try to lower this setting.");
...
#define	CATEGORY "Sound\\Win"
	...
	AddBoolC("MuteFrameAdvance", GUI.FAMute, false, "true to prevent SNES Emu Ai from outputting sound when the Frame Advance command is in use");
...
void WinLockConfigFile ()
{
	...
	fname=S9xGetDirectory(DEFAULT_DIR);
	fname+=SLASH_STR "SnesEmuAi.conf";
	STREAM fp;
