...
static HBITMAP g_startupLogoBitmap = NULL;

static void DrawStartupLogoInClientArea()
{
	if (!GUI.hWnd)
		return;
	...
	if (!g_startupLogoBitmap)
	{
		g_startupLogoBitmap = (HBITMAP)LoadImage(
			g_hInst,
	...
	{
		BITMAP bm = {0};
		GetObject(g_startupLogoBitmap, sizeof(bm), &bm);
		if (bm.bmWidth > 0 && bm.bmHeight > 0)
		{
			const double fill = 0.85;
			const int availW = max(1, (int)(clientW * fill + 0.5));
			const int availH = max(1, (int)(clientH * fill + 0.5));

			double scale = min((double)availW / (double)bm.bmWidth,
				(double)availH / (double)bm.bmHeight);
			const int drawW = max(1, (int)(bm.bmWidth * scale + 0.5));
			const int drawH = max(1, (int)(bm.bmHeight * scale + 0.5));
			...
			if (memDC)
			{
				HGDIOBJ oldBmp = SelectObject(memDC, g_startupLogoBitmap);
				SetStretchBltMode(hdc, HALFTONE);
				SetBrushOrgEx(hdc, 0, 0, NULL);
				StretchBlt(hdc, drawX, drawY, drawW, drawH, memDC, 0, 0, bm.bmWidth, bm.bmHeight, SRCCOPY);
...
bool WinDisplayReset(void)
{
	...
	switch(GUI.outputMethod) {
		...
		case OPENGL:
			if (VulkanUsed)
			{
				MessageBox(GUI.hWnd, TEXT("Changing to OpenGL requires a restart if you've already used Vulkan"), TEXT("SNES Emu Ai Display Driver"), MB_OK);
				break;
			}
		...
		case VULKAN:
			if (OpenGLUsed)
			{
				MessageBox(GUI.hWnd, TEXT("Changing to Vulkan requires a restart if you've already used OpenGL"), TEXT("SNES Emu Ai Display Driver"), MB_OK);
				break;
			}
	...
	if (!initialized) {
		...
		MessageBox(GUI.hWnd, msg, TEXT("SNES Emu Ai Display Driver"), MB_OK | MB_ICONERROR);
	...
	if (initialized) {
		...
		MessageBox (GUI.hWnd, Languages[ GUI.Language].errInitDD, TEXT("SNES Emu Ai - Display Failure"), MB_OK | MB_ICONSTOP);
		return false;
	}
}
...
void RestoreGUIDisplay ()
{
    ...
    {
		MessageBox (GUI.hWnd, Languages[ GUI.Language].errModeDD, TEXT("SNES Emu Ai - DirectDraw(1)"), MB_OK | MB_ICONSTOP);
...
void RestoreSNESDisplay ()
{
    ...
    {
		MessageBox (GUI.hWnd, Languages[ GUI.Language].errModeDD, TEXT("SNES Emu Ai - DirectDraw(4)"), MB_OK | MB_ICONSTOP);
        return;
    }
